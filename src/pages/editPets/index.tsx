...
export const EditPet = () => {
  ...
  const { fetchEditPet, getSelectedPet, fetchDeletePet } = useSelectedPets();
  const initialReport = {
    name: getSelectedPet()?.name || "",
    photoURL: getSelectedPet()?.photoURL || "",
    lat: getSelectedPet()?.lat || 0,
    lng: getSelectedPet()?.lng || 0,
    lastLocation: getSelectedPet()?.lastLocation || "",
  };
  ...
  const handleDeleteSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const pet = getSelectedPet();
    try {
      await fetchDeletePet(pet!.photoURL, pet!.id);
      console.log("Mascota eliminada correctamente");
    } catch (err) {
      Swal.fire({
        icon: "error",
        title: "Hubo un error al eliminar la mascota",
      });
      console.error(err);
    }
    Swal.fire({
